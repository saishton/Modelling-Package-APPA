...
while currenttime<runtime
    [currenttriangles,m_lpm] = extractTriangles(times,nodes,currenttime,linkprobmatrix);
    targettriangles = gamrnd(0.370,2.1564);
    %TARGETTRIANGLES should be set to the desired distribution for the
    % triangle count
    if (currenttriangles<targettriangles)&&(sum(sum(m_lpm))~=0)
        %Triangle-preferred link selection process
        accept = 0;
        while accept == 0
            [ri,rj] = chooselink(m_lpm);
...
sampleCSV2(times,nodes,runtime,cut);
